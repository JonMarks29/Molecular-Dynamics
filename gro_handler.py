...
#sysfile = "C:/Users/jmmac/Desktop/md/fusa_edge_oant/prot-lipw.gro"
remp_cg  = "C:/Users/jmmac/Desktop/md/single_remp_clean.gro"
  
remp_atom = "C:/Users/jmmac/Downloads/charmm-gui-3156411683/single_remp_atom.gro"

file = "C:/Users/jmmac/Desktop/md/fusa_100oant/fusa_oant100_r3.part0002.gro"


g = Gro(file, exclude=['W','ION'])

count = 0
o_pos_all, s_pos_all = [], []
for res in g.residues.values():
    if res.resname == 'OANT':
        o_pos, s_pos = np.inf, np.inf
        for atomnumb in res.atoms:
            if g.atoms[atomnumb].atomname[0] == 'O':
                if g.atoms[atomnumb].z < o_pos:
                    o_pos = g.atoms[atomnumb].z
            if g.atoms[atomnumb].atomname[0] == 'S':
                if g.atoms[atomnumb].z < s_pos:
                    s_pos = g.atoms[atomnumb].z

        o_pos_all.append(o_pos)
        s_pos_all.append(s_pos)

    


# for atom in cg.atoms.values():
#     if atom.atomname in ["PO1"]:
#         xc,yc,zc = atom.coor
        
# for atom in atomistic.atoms.values():
#     if atom.atomname in ["PA"]:
#         xa,ya,za = atom.coor
...
# cg.write("C:/Users/jmmac/Downloads/charmm-gui-3156411683/single_remp_cg_modified.gro")

# for coor in [(5, 2.4, 20), (5,3.5,20)]:
#     sys.replace_residue(coor=coor, original="DLPE", replace=oant, n=4, flip=True, dm=3, solvent_clash=True)
    
# sys.residue_count()
# sys.tidy_numbers()
# sys.write("C:/Users/jmmac/Desktop/md/fusa_edge_oant/prot-lipw-modified.gro")
